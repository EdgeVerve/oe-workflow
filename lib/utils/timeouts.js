...
exports.removeAllTimeouts = function removeAllTimeouts(processInstance, timerToKeep) {
  ...
  processInstance._processTimerEvents.timerIds = {
    timerToKeep: processInstance._processTimerEvents.timerIds[timerToKeep]
  };
};
...
exports.addBoundaryTimerEvent = function addBoundaryTimerEvent(delta, options, ProcessInstance, currentProcess, timerEvent) {
  ...
  var timerEventHandler = function timerEventHandler(delta) {
    ...
    currentProcess.reload(function fetchLatestPI(err, currentProcess) {
      if (err) {
      ...
      // Proposed
      /* If Timer-Token is interrupted and Process corresponding to timer event is completed*/
      if (!currentProcess || currentProcess.status === 'complete') {
        return log.warn('Process already completed. Not emitting INTERMEDIATE_CATCH_EVENT.');
      }
      // End-Proposed

      var payload = throwObject.throwObject('timer', timerEventName);
      payload.keepActive = timerEvent.keepActive;
...
var addTimerEvent = exports.addTimerEvent = function addTimerEvent(delta, timerEvent, timeoutEventHandler) {
  ...
  if (timerEvent.timeDuration) {
    ...
    if (diff > 0) {
      ...
      log.warn(log.defaultContext(), 'failed to parse timer duration, default duration of 0 ms will be applied');
      cb();
    }
  }
};
