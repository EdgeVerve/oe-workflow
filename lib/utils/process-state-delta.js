...
Delta.prototype.apply = function apply(zInstance, processDefinitionInstance, options) {
  ...
  **/
  ...
  if (this.igSeqsToExpect && typeof synchronizeFlow[this.igSeqsToExpect.gwId] === 'undefined') {
    var sfObj = {};
    var sFlows = this.igSeqsToExpect.seqFlows;
    for (var k = 0; k < sFlows.length; k++) {
      var sf = sFlows[k];
      sfObj[sf] = null;
    }
    synchronizeFlow[this.igSeqsToExpect.gwId] = sfObj;
  }
  ...
  if (this.igSeqToFinish) {
    ...
    if (nullSeq.length) {
      ...
      let flowObjects = processDefinitionInstance.processDefinition.flowObjects;
      let incomingGate = getMatchingGateway(tokens[refToken], incomingSeq, allSeq, flowObjects);    
      for (var seq in nullSeq) {
        var ifActive = isActivePath(nullSeq[seq], allSeq, incomingGate);
        if (!ifActive) {
  ...
  function getMatchingGateway(token, incomingSeqIds, allSeq,  flowObjects) {
    ...
    while(!inGate) {
      ...
      if(!inGate) {
        ...
        let maxLength = allIncomingGates.map(i => i.length).reduce(function(i, n){ return i>n?i:n});
    ...
    if(!inGate){
      ...
      let tokenBaseName = token.name.slice(-3);
    ...
    function getUpstreamGates(fromSequenceId, allSeq, flowObjects) {
      ...
      retArr.concat(getUpstreamGates(beforeSeq.bpmnId, allSeq, flowObjects));
      return retArr;
    }
  };

  function isActivePath(sequenceId, allSeq, incomingGate) {
    ...
    if (sourceToken) {
      ...
      activePath = isActivePath(beforeSeq.bpmnId, allSeq, incomingGate);
    }
