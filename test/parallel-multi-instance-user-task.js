...
describe('Test case1 for Multi Instance Parallel', function callback() {
  this.timeout(20000);
  var name = 'ParallelMultiInstanceUserTask';
  var testVars = {};
  ...
  it('complete task 2', function callback(done) {
    testVars.tasks[1].completeTask({}, {}, bootstrap.defaultContext, done);
  });
  ...
  it('complete task 3', function callback(done) {
    testVars.tasks[2].completeTask({}, {}, bootstrap.defaultContext, function callback(err) {
      if (err) {
        done(err);
      }
