...
function createUser(connection, cb) {
  ...
  connection.execute(sql, function testFunction(e, r) {
    if (e) {
    ...
    connection.execute(sql, function testFunction(err, result) {
      if (err) {
        throw new Error('Unable to create user ' + userName + ' Error :' + err);
      }
      console.log(sql, ' ......... ok');
      async.each(grants, function testFunction(g, callback) {
        var sql = 'GRANT ' + g + ' to "' + userName + '"';

        connection.execute(sql, function testFunction(err2, result2) {
          if (err2) {
...
function dropTables(cb) {
  ...
  oracledb.getConnection(
    ...
    function testFunction(err, connection) {
      if (err) {
      ...
      connection.execute(sql, {}, {maxRows: totalRows}, function testFunction(err, result) {
        if (err) {
          throw new Error('Unable to find tables ' + userName + ' Error :' + err);
        }
        connection.execute(sql, {}, {maxRows: totalRows}, function testFunction(err2, result2) {
          if (err2) {
          ...
          async.each(result2.rows, function testFunction(row, callback) {
            var sql = row[0];
            connection.execute(sql, function testFunction(err2, result2) {
              if (err2) {
    ...
    connection.execute(sql,
      function testFunction(err, result) {
        ...
        if (!result.rows || result.rows.length == 0) {
          createUser(connection, function testFunction(err) {
            if (err) {
          ...
          dropTables(function testFunction(err) {
            if (err) {
              return process.exit(1);
            }
            return process.exit();
