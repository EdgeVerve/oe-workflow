...
describe('Signal Event Boundary Interrupting Tests', function CB() {
  let workflowName = 'signal-boundary-interrupting';
  ...
  it('Throw interrupts the Catch Boundary task', function testFunction(done) {
    ...
    bootstrap.onComplete(workflowName, function testFunction(err, instance) {
      ...
      stateVerifier.verifyTokens(instance, ['Start', 'PGway', 'UserTaskThrow', 'SignalThrow', 'End1', {
        name: 'UserTaskCatch',
        status: Status.INTERRUPTED
      }, 'SignalCatch', 'TaskC', 'End3']);
      done();
    });
  });
